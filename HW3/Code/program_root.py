...
def read_data():
    ...
    x1 = []
    for row in dataset:
        dataset_value_label.append(
            [[float(row[0].replace("'", "")), float(row[1].replace("'", ""))], int(row[2].replace("'", ""))])
...
def main():
    ...
    predicted_dataset_value_label_test = test(W, b, dataset_value_label_test)
    # print(dataset_value_label_test)
    print(dataset)
    for i in range(0, len(predicted_dataset_value_label_test)):
        # print(dataset_value_label_test[i][1])
        if predicted_dataset_value_label_test[i][1] == 0:
            y0.append(predicted_dataset_value_label_test[i][0][1])
            x0.append(predicted_dataset_value_label_test[i][0][0])
        else:
            y1.append(predicted_dataset_value_label_test[i][0][1])
            x1.append(predicted_dataset_value_label_test[i][0][0])
    ...
    correct_pred=0
    for i in range(0, len(predicted_dataset_value_label_test)):
        if  predicted_dataset_value_label_test[i][1] ==  dataset_value_label_test[i][1]:
            correct_pred += 1

    accuracy = correct_pred / len(predicted_dataset_value_label_test)
    print(accuracy)
